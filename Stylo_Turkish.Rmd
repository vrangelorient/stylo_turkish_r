...
#Sys.setenv(JAVA_HOME="~/renv/local/Java/jre1.8.0_291")
#library("rJava")
...
data.frame(name = names(speechListClean1), speech = unlist(speechListClean1), stringsAsFactors = FALSE) %>%
		...
		group_by(name) %>%
		summarise(`Уникальные слова` = length(unique(word)), `Все слова` = length(word)) %>%
		ungroup() %>%
		...
		xlab("Номер речи") + ylab("") +
		ggtitle("Сколько слов президент Турции использует в своих речах?\n") +
		theme_bw() +
...
data.frame(name = names(speechListClean1), speech = unlist(speechListClean1), stringsAsFactors = FALSE) %>%
		...
		group_by(name) %>%
		summarise(`Медиана количества слов в предложении` = median(totalCharSentence), `Среднее количества слов в предложении` = mean(totalCharSentence)) %>%
		gather(variable, value, -name) %>%
		arrange(name) %>%
		...
		xlab("Номер речи") + ylab("") +
		ggtitle("Сколько слов в предложениях турецкого президента?\n") +
		theme_bw() +
...
xlab("") + ylab("") +
ggtitle("Консенсусная сеть речей президента Турции") +
...
guides(alpha = guide_legend(title = "Стилистическая близость", title.position = "top", title.hjust = 1, title.vjust = 1, ticks = FALSE, direction = "horizontal", label = FALSE, keywidth = 3.1, keyheight = 1))
